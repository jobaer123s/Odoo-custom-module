...
class HrPayslip(models.Model):
    ...
    gross_salary = fields.Float(string='Gross Salary', store=True, readonly=True, compute='_amount_all', tracking=True)
    deductions = fields.Float(string='Deduction', store=True, default=0.0, tracking=True, readonly=True)
    net_salary = fields.Float(string='Net Salary', store=True, readonly=True, compute='_amount_all', tracking=True)

    @api.depends('line_ids.total')
    def _amount_all(self):
        """
        Compute the Gross, Deduction & Net of the Salary.
        """
        for payslip in self:
            ...
            for line in payslip.line_ids:
                if c < len(payslip.line_ids) - 1:
                    total_gross_salary += line.total
                elif c == len(payslip.line_ids) - 1:
...
class HrPayslipLine(models.Model):
    ...
    amount = fields.Float(digits=dp.get_precision('Payroll'), store=True)
