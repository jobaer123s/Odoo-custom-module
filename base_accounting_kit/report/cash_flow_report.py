...
class ReportFinancial(models.AbstractModel):
    ...
    def _compute_account_balance(self, accounts):
        ...
        if accounts:
            ...
            acc_obj = self.env['account.account'].sudo()
            ...
            for row in self.env.cr.dictfetchall():
                ...
                acc = acc_obj.browse(row['id'])
                row['report_by'] = acc.report_by
            ...
            if report.type == 'accounts':
                ...
                for values in res[report.id]['account'].values():
                    for field in fields:
                        ...
                        if 'report_by' in values:
                            if values['report_by'] == 'debit':
                                values['credit'] = 0
                            elif values['report_by'] == 'credit':
                                values['debit'] = 0

                            values['balance'] = values['credit'] - values['debit']
    ...
    def get_account_lines(self, data):
        ...
        for line in tmp_lines:
            ...
            if 'sum_fr_ids' in line:
                for item in line['sum_fr_ids']:

                    line['balance'] += next((rep['balance'] for rep in lines if 'report_id' in rep and rep['report_id'] == item), 0)
