    ...
    def view_report_pdf(self):
        ...
        data['form'] = self.read(
            ['date_from', 'enable_filter', 'debit_credit', 'date_to',
             ...
             'company_id', 'date_from_cmp', 'date_to_cmp', 'filter_cmp', 'label_filter'])[0]
        ...
        # -------------------------------
        """
        if data['form']['enable_filter'] and data['form']['filter_cmp']=="filter_date":
            comp_form = data['form']
            comp_form['date_from'] = data['form']['date_from_cmp']
            comp_form['date_to'] = data['form']['date_to_cmp']
            comp_report_lines = self.get_account_lines(comp_form)
        """

        tmp_lines = report_lines
        for line in report_lines:

            if 'z_view_rule' in line and line['balance'] == 0.0 and line['z_view_rule'] == 'blank':
                line['balance'] = ' '
            elif 'z_view_rule' in line and line['balance'] == 0.0 and line['z_view_rule'] == 'dash':
                line['balance'] = '-'
            elif line['balance'] < 0.0:
                line['balance'] = '('+str(line['balance'] * -1)+')'

            if 'balance_cmp' in line:
                if 'z_view_rule' in line and line['balance_cmp'] == 0.0 and line['z_view_rule'] == 'blank':
                    line['balance_cmp'] = ' '
                elif 'z_view_rule' in line and line['balance_cmp'] == 0.0 and line['z_view_rule'] == 'dash':
                    line['balance_cmp'] = '-'
                elif line['balance_cmp'] < 0.0:
                    line['balance_cmp'] = '('+str(line['balance_cmp'] * -1)+')'

        # if sequence 0 then name and balance will not show
        for report_line in data['report_lines']:
            if report_line['sequence'] == 0:
                report_line['name'] = ''
                report_line['balance'] = ''
                report_line['debit'] = ''
                report_line['credit'] = ''

        fin_rep_obj = self.env['account.financial.report'].sudo()
        fin_rep = fin_rep_obj.browse([data['form']['account_report_id'][0]])
        data['report_data'] = {
            'report_year_title': fin_rep.report_year_title,
            'report_comment': fin_rep.report_comment
        }
    ...
    def get_account_lines(self, data):
        ...
        for report in child_reports:
            ...
            vals = {
                ...
                'notes': report.notes if report.notes > 0 else '',
                ...
                'z_view_rule': report.z_view_rule,
                'sum_fr_ids': [rep.report.id for rep in report.sum_reports if report.sum_reports],
                'border_top': report.border_top,
                'border_left': report.border_left,
                'border_bottom': report.border_bottom,
                'border_right': report.border_right,
                'border_bottom_double': report.border_bottom_double
            }
